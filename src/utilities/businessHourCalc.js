...
//  for a 24 hour business day, businessStart = 00:00:00 businessEnd = 23:59:59 (or an equivalent
//    timescale e.g. businessStart = 13:00:00 businessEnd = 12:59:59)

//  holidays is an array of business holiday timestamps at midnight of the holiday from the same
//    timezone as the startingTime (formatted ["YYYY-MM-DD HH:MM:SS"])
...
const businessDaysCalculator = (
  ...
  const milliBusinessEnd =
    timeMaker(businessEnd) < timeMaker(businessStart)
      ? timeMaker(businessEnd) + DAY
      : timeMaker(businessEnd);
  const endTime =
    timeMaker(endingTime.split(" ")[1]) + DAY < milliBusinessEnd
      ? timeMaker(endingTime.split(" ")[1]) + DAY
      : timeMaker(endingTime.split(" ")[1]);
  ...
  const isHoliday = (day) =>
    holidaylist.filter(
      (holiday) =>
        holiday.getTime() <= day.getTime() + milliBusinessStart &&
        day.getTime() + milliBusinessStart <= holiday.getTime() + DAY
    ).length === 0
      ? false
      : true;
  ...
  if (
    endDate.getTime() + endTime < startDate.getTime() + startTime ||
    endDate < startDate
  ) {
  ...
  if (
    startDate.getTime() + milliBusinessEnd >=
    endDate.getTime() + timeMaker(endingTime.split(" ")[1])
  ) {
  ...
  if (!isHoliday(endDate)) {
    ...
    if (
      endTime < milliBusinessEnd &&
      endTime > milliBusinessStart &&
      endTime < DAY
    ) {
      count += endTime - milliBusinessStart;
